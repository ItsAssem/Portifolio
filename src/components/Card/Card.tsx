  ...
  return (
    <div className="CardContainer flex flex-col justify-between bg-black/40 backdrop-blur-md h-[500px] outline-1 p-8 w-full max-w-md rounded-2xl border-2 border-green-500/50 shadow-lg whitespace-pre-line animate-liquid-glow drop-shadow-[0_0_25px_rgba(34,197,94,0.4)] mx-4 my-4">
      <div className="flex flex-col gap-4 shrink-0">
        <h2 className="CardTitle text-center text-lg sm:text-xl md:text-2xl font-bold text-green-400 animate-text-glow whitespace-normal">
          {title}
        </h2>
        ...
        <div className="flex flex-wrap gap-2">
          {tags.map((tag, index) => (
            <span
              className="inline-block bg-[#00df9a]/30 text-green-300 text-xs sm:text-sm font-medium px-3 py-1 rounded-full hover:bg-green-800/40 backdrop-blur-sm"
              key={index}
            >
              {tag}
            </span>
      ...
      <div className="flex flex-col gap-4 flex-1 min-h-0">
        <p className="CardDesc overflow-y-auto text-green-100/70 leading-relaxed pr-2">
          {description}
        </p>
